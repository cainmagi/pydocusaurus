...
在开发阶段，GitHub 上的版本有可能比在PyPI发布的版本新。如果用户需要安装 GitHub 版本，请确保已经安装了[Git<IconExternalLink/>][link-git]。

确保前置软件准备无虞后，运行下列命令安装开发者版本：
...
若有需要，可以在`pip install`命令的后面加上`-e`。这将令所安装的模块支持即时修改。亦即，对从 GitHub 上 clone 的项目做出的任何改动都将反馈到全局安装的`pydocusaurus`模块包上。
...
确保前置软件准备无虞后，请按照以下命令 clone 项目并运行测试：
...
本项目只兼容到`python>=3.14`版本。如果你发现你所使用的版本无法通过单元测试，请提交问题报告：
...
[link-pytest]: https://pytest.cn/en/stable/
